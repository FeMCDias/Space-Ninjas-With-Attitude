...
class level():
    ...
    # Função que cria os sprites das madeiras e retorna um grupo de sprites
    ...
    def atualiza_inimigo_e_confere_vitoria(self):
        ...
        if self.colisao_quadrados(self.ball.posicao[0], self.ball.posicao[1],self.ball.width, self.ball.height, self.enemy.x, self.enemy.y, 100, 100):
            self.assets['cut_enemy_sound'].set_volume(0.25) #Se colidir com o inimigo, toca o som de cortar o inimigo
            ...
            if self.enemy.health <= 0:
                ...
                if self.level == 4:
                    self.victory = True
                    self.level = 3
                self.enemy.change_level(self.level) #Muda o level do inimigo
                self.madeiras_sprite = self.cria_sprites_e_madeiras(2) #Cria novas madeiras
                self.ball.posicao = self.posicao_inicial
                self.ball.reset_ball() #Reseta a bola, atributos, etc
                self.ball.change_type(self.state['weapon'], self.level) #Muda o tipo da bola, de acordo com a arma escolhida
